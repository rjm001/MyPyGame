...
#oBackgroundImage = pygwidgets.Image(window, (0, 0), 'images/background.jpg') #note, doesn't automatically resize the background!
oBall = Ball(window, WINDOW_WIDTH, WINDOW_HEIGHT)
oDisplayTextTitle = pygwidgets.DisplayText(window, (0, 20), 'pygwidgets example: original by Irv Kalb, embellished here', 
                                    fontSize=36, width= WINDOW_WIDTH, textColor=BLACK, justified='center')

oInputTextA = pygwidgets.InputText(window, (20, 100), 'Input text here. The next text box hides text.',
                                    textColor=WHITE, backgroundColor=BLACK,
                                    fontSize=24, width=400)

oInputTextC = pygwidgets.InputText(window, (20, 250), initialFocus=True,
                                    textColor=(0, 0, 255),
                                    fontSize=28)  # add: , mask='*' for passwords

oInputTextB = pygwidgets.InputText(window, (20, 175), initialFocus=True,
                                    textColor=(0, 0, 255),
                                    fontSize=28,mask = '*')  # add: , mask='*' for passwords
...
oDragger = pygwidgets.Dragger(window, (700, 300), 
                        'images/dragMeUp.png', 
...
oPythonIcon = pygwidgets.Image(window, (50, 500), 'images/pythonIcon.png')
...
oFrisbeeImage = pygwidgets.Image(window, (662, 100), 'images/frisbee.png')
    ...
    for event in pygame.event.get():
       # check if the event is the close button
        ...
        if oInputTextB.handleEvent(event):  # pressed Return or Enter
            ...
            print('The text of oInputTextB is not secret here! It\'s ' + theText)

        if oInputTextC.handleEvent(event):  # pressed Return or Enter
            theText = oInputTextC.getValue()
            print('The text of oInputTextC is: ' + theText)
